...
## 목적
- 특정 도메인과 무관하게 앱 개발에 공통으로 적용할 수 있는 에이전트 조직과 협업 방식을 정의한다.
...
- 최소 포함: 작업 단계, 영향 범위, 리스크, 검증 방법, 담당 에이전트.

3. 실행 에이전트 확정
- 요청 성격에 맞춰 최소 3개, 일반적으로 4~6개 에이전트를 선택한다.
- 기본 세트: PM + Architect + Engineer(Frontend/Backend) + Designer + QA
- 여러 스트림(예: 탭별 작업)을 병렬로 돌릴 때는 PM이 오케스트레이터 역할을 맡아 의존성, 대기 상태, 재개 시점을 관리한다.
- 병렬 스트림 간 질의가 필요하면 PM이 상대 스트림 에이전트를 호출해 합의안을 모은다.

4. 구현 수행
- Engineer와 Designer가 설계 계획 기준으로 작업을 수행한다.
- 계획 외 변경이 생기면 PM + Architect 재검토 후 진행한다.
- 개발 변경에는 테스트 추가/수정과 커버리지 유지가 포함되며, 기본 목표는 라인 커버리지 70% 이상이다.
...
- 개발 작업이면 QA는 테스트 통과 여부뿐 아니라 커버리지 70% 유지 여부와 예외 기록 존재 여부도 함께 확인한다.
...
- 보안 민감 작업: 보안/프라이버시 에이전트 필수 포함

## 강제 게이트 요약
- Gate A (사전): `PM + Architect` 스펙 충돌/취지 적합성 검토
- Gate B (계획): `Architect` 상세 작업 계획 확정
- Gate C (사후): `PM + Architect` 구현 결과 적합성 검토
- Gate D (품질): `QA` 테스트 승인 및 커버리지 70% 유지 확인

## 최소 운영 조합 (소규모 팀)
- 3-에이전트 조합: PM/Designer(겸직), Fullstack Engineer, QA
- 5-에이전트 조합: PM, Designer, Architect, Engineer, QA
